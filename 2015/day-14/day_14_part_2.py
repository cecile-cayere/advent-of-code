...
for line in content:
    ...
    reindeers[reindeer] = [km_s, move_time, rest_time, 0, move_time, 0, 0]
...
while i < 2503:
    ...
    for reindeer in reindeers.values():
        ...
        if(lead_distance < reindeer[3]):
            lead_distance = reindeer[3]

    for reindeer in reindeers.values():
        if(reindeer[3] == lead_distance):
            reindeer[6] = reindeer[6] + 1
